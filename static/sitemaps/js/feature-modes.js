...
showNamePrompt: true,
    ...
    onStop: function(state) {
        ...
        if (state.name && !isNull(this._ctx.store.get(state.line.id))) {
    ...
    clickAnywhere: function (state, e) {
        //updated to allow a click anywhere to create a line if something entered into name box
        var nameInput;
        try {
            if(!isNull(this._inputEl.value)) nameInput = this._inputEl.value
        }
        catch(err) {
            nameInput == null;
        }
